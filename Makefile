...
ifeq ($(UNAME), Darwin)
	...
	PUBLISH_SSH_PORT = -p 11194:11194/udp
...
openvpn-load: ## Load OpenVPN configuration file - macOS only
	@open conf/docker-dns.ovpn
...
install: welcome build-docker-image install-dependencies## Setup DNS container to resolve ENV.TLD domain inside and outside docker in your machine
	...
	@docker run -d --name $(DOCKER_CONTAINER_NAME) --restart always --privileged --security-opt apparmor:unconfined -p $(PUBLISH_IP_MASK)53:53/udp -p $(PUBLISH_IP_MASK)53:53 $(PUBLISH_SSH_PORT) -e TOP_LEVEL_DOMAIN=$(TLD) -e HOSTNAME=$(HOSTNAME) -e HOSTUNAME=$(UNAME) --volume /var/run/docker.sock:/var/run/docker.sock $(DOCKER_CONTAINER_TAG) -R
ifeq ($(UNAME), Darwin)
	@sed -i '' 's/\s#bind-interfaces//' $(DNSMASQ_LOCAL_CONF)
	@sed -i '' 's/interface=docker.*//' $(DNSMASQ_LOCAL_CONF)
	@make openvpn-load
	@echo Remember to connect Tunnelblick to docker-dns VPN! after that, your containers will be reachable using CONTAINER_NAME.$(TLD) inside and outside docker.  E.g.: nc -v $(DOCKER_CONTAINER_NAME).$(TLD) 53
else
	@echo All of your containers are reachable using CONTAINER_NAME.$(TLD) inside and outside docker.  E.g.: nc -v $(DOCKER_CONTAINER_NAME).$(TLD) 53
endif
